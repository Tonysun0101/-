...
def main():     # 主要遊戲開始了!
    ...
    while not done:     # 開始跑一個無限回圈
        ...
        if not shift_avail and time_run and first:     # 按下Shift了，開始計時，而這是這個題目第一次被計時
        ...
        if ans_turn == 1 and differ > 5 and time_run:       # 如果秒數大於三，代表這個玩家回答太久了
            ans_turn = 2                # 換成二號回答
            first = False               # 這不是第一次計時了
            d_shift = time.time()       # 計下換人回答當下的時間
            times_up = True             # 計下有人沒在時間內回答
        elif ans_turn == 2 and differ > 5 and time_run:     # 如果秒數大於三，代表這個玩家回答太久了
            ans_turn = 1                # 換成一號回答
            first = False               # 這不是第一次計時了
            d_shift = time.time()       # 計下換人回答當下的時間
            times_up = True             # 計下有人沒在時間內回答

        if times_up or change:      # 有人沒在時間內回答，或是有人回答了，都要換人，並重新計時
            d_now = time.time()                     # 記下當下時間
        ...
        for event in pygame.event.get():    # 判斷玩家做了哪些動作
            ...
            if event.type == pygame.KEYDOWN:    # 鍵盤事件
                ...
                if event.key == pygame.K_a and not shift_avail:    # 如果玩家按了shift，並且選擇了a當答案
                    change = True                   # 等等要換人回答
                ...
                elif event.key == pygame.K_b and not shift_avail:  # 如果玩家按了shift，並且選擇了b當答案
                    change = True
                    d_shift = time.time()
                ...
                elif event.key == pygame.K_c and not shift_avail:  # 如果玩家按了shift，並且選擇了c當答案
                    change = True
                    d_shift = time.time()
                ...
                elif event.key == pygame.K_d and not shift_avail:  # 如果玩家按了shift，並且選擇了d當答案
                    change = True
                    d_shift = time.time()
        ...
        if blit_timer:                                                  # 計時器顯示，前提是blit_timer == True
